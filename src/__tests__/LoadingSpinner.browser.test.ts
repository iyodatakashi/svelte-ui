...
test('LoadingSpinner size property changes reactively', async () => {
	...
	await expect
		.element(spinner)
		.toHaveAttribute('style', expect.stringContaining('--internal-loading-spinner-size: 24px'));
	...
	await expect
		.element(spinner)
		.toHaveAttribute('style', expect.stringContaining('--internal-loading-spinner-size: 32px'));
});
...
test('LoadingSpinner color property changes reactively', async () => {
	...
	await expect
		.element(spinner)
		.toHaveAttribute('style', expect.stringContaining('--internal-loading-spinner-color: red'));
	...
	await expect
		.element(spinner)
		.toHaveAttribute('style', expect.stringContaining('--internal-loading-spinner-color: blue'));
});
