...
- **Button**, **Fab**: The button no longer shrinks when entering the loading state. Previously the icon was removed from layout while loading, causing the width to change; the icon space is now preserved so the size stays constant.

## [0.0.54] - 2026-07-14

### Fixed

- **Checkbox**, **Radio**, **Switch**: The label now inherits `font-weight` from its surrounding context instead of being forced to `normal`, so labels follow the font weight of the parent element.

## [0.0.53] - 2026-07-14

### Added

- **Button**, **Fab**: New `iconPosition` prop (`'left'` | `'right'`, default `'left'`) to place the icon after the label instead of before it.

### Changed

- **Dialog**: The header/footer divider lines shown in scrollable mode now use the standard border color for better visibility.
...
- **StepNav**: In both in-page and URL modes, only reached steps (index `<= progress`) are now interactive. Users can go back to completed steps but can no longer skip ahead to steps that have not been reached yet.
...
- **Textarea**: Fixed the non-focused display view and the editable textarea wrapping text at different positions, which caused text to shift when focusing/blurring.
...
- **Skeleton**: Pulse animation is now slightly faster (4s → 3s) and has higher contrast at its low point (opacity 0.5 → 0.3).
...
- **Textarea**: `display-text` (non-focused view) no longer overflows its container when the parent element has a constrained height.
- **Textarea**: Line wrap positions now match between the non-focused view and the editing view. This also applies when `linkify` is enabled.

## [0.0.41] - 2026-06-02

### Fixed

- **MultiSelect** (inline): Dropdown icon is now correctly centered on the first line of text, and stays pinned there when items wrap to multiple lines.
...
- **MultiSelect**: Renamed `values` prop to `value` for consistency with other form components. Use `bind:value` for two-way binding.
- **MultiSelect**: Removed unused `name` prop.
...
- **Combobox**: Initial `value` prop is now correctly reflected in the displayed text.
- **Combobox**: Changing `value` externally now updates the displayed text (when not focused).
- **Combobox**: `inline` mode now shows the dropdown arrow icon, matching `Select` behavior. The icon position is also corrected to align with the text.

## [0.0.36] - 2026-05-13

### Fixed

- **CheckboxGroup**: Changing `value` externally (e.g., resetting from the parent) now correctly updates the checked state of each checkbox.
- **CheckboxGroup**: `option.disabled` is now correctly applied per option.

## [0.0.35] - 2026-05-13

### Fixed

- **CheckboxGroup**: Checkboxes now render correctly when `options` is set dynamically, and the initial `value` is now correctly reflected as pre-checked on first render.
